...
SCREENWIDTH = 289
SCREENHEIGHT = 511
# Initialise a window for the game
...
def getRandomPipe():
    """
    Generate positions for two pipes(bottom and top)
    """
...
def welcomeScreen():
    ...
    while True:
        ...
        for event in pygame.event.get():
            # user clicks cross button, close the game
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                pygame.quit()
                sys.exit()

            # user clicks space key or up arrow key then start the game
            elif event.type == KEYDOWN and (event.key == K_SPACE or event.key == K_UP):
                # Return to main where game is launched
                return
            else:
                ...
                pygame.display.update()  # The screen will be updated only after this command
                FPSCLOCK.tick(FPS)
...
def mainGame():
    ...
    playerFlapped = False  # will be set true only when the bird is flapping
    ...
    while True:
        ...
        # returns true if the player crashes into the pipes
        ...
        # Lets blit our sprites now ( Blit means to render a image on window by resizing it)
        SCREEN.blit(GAME_SPRITES['background'], (0, 0))
        for upperPipe, lowerPipe in zip(upperPipes, lowerPipes):
...
if __name__ == "__main__":
    ...
    pygame.init()
    FPSCLOCK = pygame.time.Clock()
    pygame.display.set_caption('Flappy Phani')

    # Create a tuple to load the images
    GAME_SPRITES['numbers'] = (
        # load the image and convert_alpha will optimise the image by providing faster rendering
    ...
    # Tuple with initial message to start
    GAME_SPRITES['message'] = pygame.image.load(
        'sprites/message.png').convert_alpha()

    # Tuple to store the base image of the game
    GAME_SPRITES['base'] = pygame.image.load(
        'sprites/base.png').convert_alpha()

    # Tuple with the image of pipe, we need two pipes. One on the top and the other in the bottom. (Straight pipe and reversed pipe)
    GAME_SPRITES['pipe'] = (
